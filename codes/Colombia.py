...
def Col():
    ...
    for x in range(len(fe['fecha'])):
        if (fe['fecha'][x] == 3):
            cant[0] += int(fe['cantidad'][x])
        elif (fe['fecha'][x] == 4):
            cant[1] += int(fe['cantidad'][x])
        elif (fe['fecha'][x] == 5):
            cant[2] += int(fe['cantidad'][x])
        elif (fe['fecha'][x] == 6):
            cant[3] += int(fe['cantidad'][x])
        elif (fe['fecha'][x] == 7):
            cant[4] += int(fe['cantidad'][x])
        elif (fe['fecha'][x] == 8):
            cant[5] += int(fe['cantidad'][x])
        elif (fe['fecha'][x] == 9):
            cant[6] += int(fe['cantidad'][x])
        elif (fe['fecha'][x] == 10):
            cant[7] += int(fe['cantidad'][x])
        elif (fe['fecha'][x] == 11):
            cant[8] += int(fe['cantidad'][x])

    #Gráfica de Barras Casos Por Mes
    fig, ax = pt.subplots(figsize=(20,10))
    ax.set_ylabel('NÚMERO DE CASOS')
    ax.set_title('CASOS CONFIRMADOS DE COVID-19 EN COLOMBIA POR MESES')
    pt.plot(meses, cant,marker="o")
    fig.tight_layout()
